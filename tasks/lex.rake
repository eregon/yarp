    ...
    Dir["#{TOP_100_GEMS_DIR}/**/*.rb"].each do |filepath|
      test filepath do
        ...
        unless result.success?
          if system(RbConfig.ruby, "-c", filepath, out: File::NULL, err: File::NULL)
            raise "#{filepath} had errors but CRuby parsed it fine: #{result.errors}"
          else
            puts "\n#{filepath} was ignored as it is not valid syntax"
            next
          end
        end
        value = result.value
