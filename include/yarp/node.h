...
static inline yp_location_t yp_location_from_start_end(const char* start, const char* end) {
  return (yp_location_t) { .start = start, .length = (end - start) };
}

#define YP_LOCATION_FROM_START_END2(start_pointer, end_pointer) (yp_location_t) { .start = (start_pointer), .length = ((end_pointer) - (start_pointer)) }
#define YP_LOCATION_FROM_START_END(start_pointer, end_pointer) yp_location_from_start_end(start_pointer, end_pointer)
